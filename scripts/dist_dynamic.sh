...
# a fine-grained tail that stealing can rebalance.  Output PNGs keep the
# global frame index, so collection stays a plain union.
#
# Usage:
...
for r in $(seq 0 $((N - 1))); do
  ...
  start=$((start + cnt))
  echo "[dyn] rank $r initial share: $cnt frames (weight ${W[r]}/$WSUM)"
done
...
take_chunk() {
  ...
  (
    flock 9
    local own="$BAG/rank$r.list" n k
    n=$(wc -l < "$own")
    if [[ "$n" -gt 0 ]]; then
      k=$(( (n + 1) / 2 )); [[ "$k" -lt "$KMIN" ]] && k=$n
