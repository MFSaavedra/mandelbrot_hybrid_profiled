...
further to \texttt{diffT}\,=\,$0.05$ does worse (53.45~s with
...
the worst-case CPU region barely moves at all: it remains a
...
\begin{itemize}
    ...
    \texttt{experiments/05-fig1115-postfix} run rather than re-measured, to
...
\insertimage{../experiments/06-difft-postfix/difft_sweep.png}{width=\linewidth}
...
\begin{table}[H]
    ...
    \begin{tabular}{rrlrr}
        \hline
        diffT & Wall (s) & Worst CPU region & Worst kernel (ms) & Worst kernel size \\
        \hline
...
at every \texttt{diffT} tested, taking 14.0--15.5~s. Five
...
The corner-spread heuristic cannot see this region as a split
...
identical, not just close. The diffT knob controls how much
near-uniformity is acceptable; it cannot reject exact
...
\begin{enumerate}
    \item More uniform region sizes mean the tail of the leaf
    ...
    tighten.
    \item The GPU consumes leaves $\sim$40$\times$ faster than
    ...
    \item The worst-case region's $\sim$15~s wall is masked by
    ...
    15.3~s is more often below the queue-drain time and ceases
...
\begin{itemize}
    \item To split the worst region. The corner heuristic
    ...
    \item To use cardioid and bulb interior certificates. For
    ...
    the corners.
    \item Asynchronous CUDA streams. Independent of the
...
\begin{itemize}
    ...
    \item Run-to-run variance for the hybrid actually tightens
...
\texttt{04-postfix-profile.tex} (edge-midpoint sampling, cardioid
