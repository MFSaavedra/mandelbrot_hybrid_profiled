...
Binary: \texttt{0f782fd} (tag \texttt{binary-v0-buggy}) --- original code, pre-bugfix.%
}\end{center}
...
decrease as one moves from GPU-alone through hybrid configurations
...
\begin{enumerate}
    ...
    profiling captures only components of total time and not the
...
\texttt{01-initial-profile.tex}). Those mechanisms give:
...
What was not captured was the total end-to-end wall-clock time of
...
\texttt{mandelHybrid}) never exceed 12, and the count of pure CPU
...
inside each bar in white to avoid colliding visually with the
...
GPU-alone, i.e.\ faster than the GPU); the explanation is
...
\begin{table}[H]
    ...
    \begin{tabular}{lrrr}
        \hline
        Step & Time (s) & $\Delta$ time (s) & $\Delta$\,\% of GPU-alone \\
        \hline
        GPU            & 76.54 &   ---  &   --- \\
...
Amdahl-like saturation of the hybrid: once the work-queue
...
\begin{quote}
On a CPU/GPU pairing where the GPU is roughly 2$\times$ faster
