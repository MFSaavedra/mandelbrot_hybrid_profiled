...
Compares binary \texttt{0f782fd} (tag \texttt{binary-v0-buggy}, original) vs.\ \texttt{d5bf30c} (tag \texttt{binary-v1-bugfix}, post-fix).%
}\end{center}

% -----------------------------------------------------------------------
\begin{abstract}
This report repeats the \texttt{diffThreshold} sensitivity sweep of
\texttt{06-difft-sweep.tex} on the original (buggy) binary
...
for max-region data. The headline finding is that the optimal
...
fix's wall-time delta is strongly config-dependent:
CPU-only runs $\sim$1\% faster post-fix at every \texttt{diffT},
while GPU-only runs $\sim$1\% slower post-fix at every \texttt{diffT},
and the hybrid is essentially neutral (within $\pm 0.5\%$)
...
\texttt{03-bug-analysis.tex}); the binary measured in
\texttt{06-difft-sweep.tex} was the
...
\begin{itemize}
    ...
    Fig~11.15 sweep --- \texttt{experiments/02-fig1115-prefix/} for pre-fix and
    \texttt{experiments/05-fig1115-postfix/} for post-fix.
...
\begin{table}[H]
    ...
    \caption{Mean wall-clock time (s) per config and \texttt{diffT},
             ...
             $\Delta$ means the bug fix helped this
             configuration; positive means it hurt.}
    \label{tab:wall}
    \begin{tabular}{lrrrrr}
        \hline
        Config / diffT & 0.05 & 0.10 & 0.20 & 0.30 & 0.50 \\
        \hline
...
\insertimage{../experiments/07-difft-prefix/compare.png}{width=\linewidth}
...
\begin{table}[H]
    ...
    \begin{tabular}{lrrrrr}
        \hline
        diffT & 0.05 & 0.10 & 0.20 & 0.30 & 0.50 \\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{rrlrl}
        \hline
        diffT & Pre-fix maxCPU (s) & Pre size & Post-fix maxCPU (s) & Post size \\
        \hline
...
change which \texttt{diffT} to pick, only how each \texttt{diffT}
performs. The recommendation from \texttt{06-difft-sweep.tex} to lower
...
\begin{itemize}
    \item CPU12 is always faster post-fix ($-0.4\%$ to
    ...
    \item GPU is always slower post-fix ($+0.4\%$ to
...
\texttt{03-bug-analysis.tex} explained why the bug discards corner-0's
...
order in which corner values are visited, which is
...
worst-classification helps the single GPU thread because
not splitting reduces the per-launch overhead the GPU thread
...
less aggressive thresholds are now the worse choice for
GPU-only.
...
\texttt{06-difft-sweep.tex}:

\begin{enumerate}
    \item Use \texttt{diffT}\,=\,$0.1$ as the default. Both
    branches agree this is the hybrid sweet spot. The fix-induced
    overhead at \texttt{diffT}\,=\,$0.5$ disappears at this
    tighter value.
    \item Avoid \texttt{diffT}\,=\,$0.05$. Post-fix variance
    ...
    \item The corner heuristic itself is the bottleneck.
