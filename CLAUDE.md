...
Dependencies: `nvcc` (CUDA), `g++`, Qt5 (`Qt5Core Qt5Gui` via pkg-config). The Makefile autodetects the toolchain, overridably (`make CUDAINST=… HOSTCC=…`, or environment variables): `CUDAINST ?=` first existing of `/opt/cuda` (Arch) / `/usr/local/cuda` (Ubuntu/NVIDIA convention), `HOSTCC ?=` `g++-15` if present else system `g++`, and `NVCC` is invoked as `$(CUDAINST)/bin/nvcc -ccbin $(HOSTCC)` — the toolkit's own nvcc, never `PATH`'s (Ubuntu's `/usr/bin/nvcc` is a stale apt CUDA 12.0 that rejects gcc 13). The `g++-15` preference exists because Arch's system gcc 16 is unsupported by nvcc 13.2; on yeco (Ubuntu 24.04, gcc 13.3, CUDA 13.3) plain `make` picks `g++` + `/usr/local/cuda`. The same `HOSTCC` compiles the host TUs and nvcc's host half so ABI is consistent across `nvcc`/`g++` translation units.
...
`DIST_NODES` / `DIST_RANK` / `DIST_BLOCK` (environment variables; unset = off) select which frames of the interpolated sequence a process renders: frame `i` is rendered iff `(i / DIST_BLOCK) % DIST_NODES == DIST_RANK` — `DIST_BLOCK=1` pure cyclic (default), `k` block-cyclic, `>= ceil(frames/N)` contiguous block. Two-level hierarchy: frames are distributed **across** nodes; the region-level work queue stays **intra-node**, untouched. Every node reads the same `spec.in` (the corner interpolation still accumulates over *all* frame indices, so an owned frame's corners are bit-identical to a single-node run), and output PNGs keep the **global** frame index, so multi-node collection is a plain union — no renumbering, no collisions. `scripts/dist_frames.sh hosts.txt` orchestrates over SSH with GNU parallel (ships the spec, runs each rank in its `dist_work/rankN`, collects the PNGs; host `":"` = local, no SSH). Collection is a **single tar stream per rank** (not per-file scp — 2-3 protocol round trips per file dominate on WAN pairs: 6.6 s vs 1.0 s for a 79-file/36 MB share through the dichato jump; report `27`). Hosts-file lines are `host [rdir [args...]]` — per-node project root and run config (`-`/omitted = the `RDIR`/`ARGS` defaults), which is how heterogeneous nodes join: a GPU-less node builds with `make GPU=0` and runs with its own thread count and `gpuEnable=0` (e.g. `ivy /home/lynx/box/mandelbrot_hybrid_profiled 8 0 0.1 32768 1 1`), and a fast hybrid node joins the same way (e.g. `yeco /home/mfsaaved/box/mandelbrot_hybrid_profiled 20 1 0.1 32768 1 1` — the RTX 4090 node behind the dichato ProxyJump, `ssh yeco` alias). `experiments/25-frame-distribution/verify_dist.sh` is the byte-identity check — executor-pinned (CPU-only and GPU-only), mirroring experiments 21/23, because CPU-vs-CUDA FP64 last-ulp rounding makes hybrid output timing-dependent.

Two further ownership modes sit above the block-cyclic predicate (precedence: `DIST_FRAMES` > `DIST_WEIGHTS` > block-cyclic). `DIST_WEIGHTS="5,1"` (+ `DIST_SEED`, default 1234) is **seeded weighted-random static assignment**: frame `i` belongs to the rank whose cumulative-weight interval contains `splitmix64(seed+i) mod W` — pure uint64 arithmetic, so every rank on every machine computes the same partition with no communication; rank `r` owns ~`F·w_r/W` frames randomly interleaved along the cost trend (pass `WEIGHTS=5,1` to `dist_frames.sh`). `DIST_FRAMES="3,17,42"` renders an explicit global frame list — the dispenser interface. `scripts/dist_dynamic.sh hosts.txt` (env `WEIGHTS`/`SEED`/`KMIN`) builds on it: **coordinator-driven work stealing** — a seeded shuffle dealt proportionally into per-rank bags, per-node drivers dispatching guided chunks (half of remaining, min `KMIN`) as repeated `DIST_FRAMES` invocations, and a node whose bag empties steals half the richest node's undispatched tail (flock-serialized; each frame renders exactly once; no node-to-node sockets — everything rides the coordinator's SSH channel).
...
└── scripts/
    ...
    ├── plot_fig1115.py
    ├── dist_frames.sh         multi-node frame distribution, static (GNU parallel over SSH)
    ├── dist_dynamic.sh        weighted-random init + work-stealing coordinator (DIST_FRAMES chunks)
...
- `feat/frame-distribution` — **merged into `main`** (merge commit `b342303`; the conflict with `main`'s Doxygen twin `a4a5657` was resolved by taking the branch tree, which supersedes it — the merged tree is byte-identical to the branch's; tag `binary-frame-dist`). Static cyclic/block-cyclic **multi-machine frame distribution** — the two-level hierarchy: frames across nodes, region queue intra-node (untouched). `DIST_NODES`/`DIST_RANK`/`DIST_BLOCK` env vars gate frame ownership via a single predicate in `main.cpp`'s frame loop (`(i/BLOCK)%N==RANK`); non-owned frames get no `MandelFrame` and no root region (per-node frame-buffer memory drops to owned frames), while the corner interpolation still accumulates over all indices so owned frames are bit-identical to a single-node run. Filenames keep the global frame index → collection is a plain union. Orchestration: `scripts/dist_frames.sh` (GNU parallel over SSH). Identity validated on the production spec: 3 simulated local nodes × {CPU-only cyclic, GPU-only cyclic, CPU-only block-cyclic(8)}, every check 100/100 frames byte-identical to the single-process reference (`experiments/25-frame-distribution/`); plus a real 2-node laptop+ivy run over SSH (ivy = Haswell 8T, `make GPU=0` build), 100/100 byte-identical — cross-compiler/µarch CPU identity holds at plain `-O2` (no `-march`, no FMA contraction). Ivy's 3.2× slower share is the heterogeneity the static assignment can't absorb (dispenser motivation). The manager-worker frame dispenser (one message per frame, for heterogeneous nodes / monster frames) is deliberately **not built**; the seam is the ownership predicate — `WorkQueue::append` is already mutex-safe for incremental insertion, but a dispenser must handle `CalcThr::run`'s exit-on-empty-queue. Report `25` (on this branch) covers the mechanism, the identity validation, and the **heterogeneous 2-node benchmark** (laptop + ivy, `make GPU=0` CPU-only node): every equal-share static config loses to the laptop alone (best +51%) — 4.56× per-frame throughput gap, analytic ceiling 1.22×, half eaten by 3.4–4.2 s ship/collect overhead → slow nodes buy capacity, not latency. The homogeneous block/cyclic/block-cyclic interleave study (report 13's original earmark) remains open — needs comparable nodes. Post-report-25 the branch adds **weighted static + dynamic balancing** (`ffe61aa`+): `DIST_WEIGHTS`/`DIST_SEED` (seeded splitmix64 weighted-random partition, communication-free) and `DIST_FRAMES` (explicit list) in the binary, plus `scripts/dist_dynamic.sh` — coordinator-driven work stealing (guided half-remaining chunks per node via `DIST_FRAMES`, empty-bag nodes steal half the richest undispatched tail under flock). Identity revalidated at production scale on laptop+ivy CPU-pinned: weighted static 100/100, dynamic-with-stealing 100/100 — including a live steal correcting miscalibrated weights (CPU-pinned laptop is only ~2.4× ivy, so 5:1 overloaded it and ivy stole 5 frames back). Benchmark (`bench_dynamic.sh`/`results_dynamic.csv`, hybrid laptop + CPU ivy): right weights (5:1) reach **baseline parity** — weighted static 22.93 s, dynamic 22.01 s vs a thermally noisy 21.06±2.73 baseline (the report-25 ceiling realized; vs 46.90 s equal-share); wrong weights (1:1) recover **72% of the miscalibration penalty autonomously** (46.90 → 28.18 s) once chunks are capped at `KCAP·w_r` frames (uncapped, a slow node's half-bag chunk — 25 frames = 27.4 s — is in flight and unstealable; the weight-proportional cap bounds every node's exposure at the same wall-time budget). With calibrated weights stealing never fires (pure insurance). Report `26` (on this branch) writes up the dynamic work: mechanism, the KCAP defect-and-fix, both benchmark runs (`results_dynamic_uncapped.csv` preserves the pre-cap run), and the corrective-steal finding. Post-report-26 the branch adds the **yeco campaign + collection batching** (`c8ac920`/`1b5da5f`/`f310656`, report `27`): both protocols rerun with yeco (Ultra 7 265K 20T + RTX 4090, via the dichato SSH jump) as a remote node **4.1× faster** than the laptop — equal shares now strand the *laptop* (all 2-node configs beat the laptop baseline, all lose to yeco solo 5.15 s); weighted 1:4 computes at the 4.14 s harmonic ideal on both nodes yet e2e was 13.11 s because balanced compute *exposes* per-file scp collection (~68 ms/file at ~25 ms RTT — hidden behind the stranded node in every equal-share config). Fix: collection batched as one tar stream per rank (`dist_frames.sh` + `dist_dynamic.sh`, measured 6.9× on the transfer) → weighted static **6.45±0.26 s** (−51%, σ ×7 tighter) = parity with yeco-solo+fetch — the capacity-not-latency ceiling realized a third time. Dynamic stealing self-schedules on *lane time* (compute+shipping; each chunk's copy sits before the next `take_chunk`): under scp the slow laptop steals 22 frames from the fast node — the lane-optimal split; post-batch, right weights make stealing a +21% chunking premium (fresh CUDA context per chunk — was free insurance on CPU-only LAN ivy), and wrong 1:1 weights show `KCAP·w_r` throttling the *fast* node (9 dispatches for 63 frames — the cap inherits weight miscalibration; rate-adaptive caps are the identified follow-up). Post-report-27 the branch adds **report `28`** (analysis, no renderer run): the yeco campaign's constants fed into the sibling `../DLTlib`'s collection-aware DLT solver (`SolveImage`, a=0/c=1) — l=0 reproduces the empirical 1:4 weights, the scp optimum (39/61) retrodicts the collection wall and the stealer's 42/58 equilibrium (3 frames off closed form), post-batch 1:4 is 2% (sub-σ) from optimal, and transport-beats-scheduling falls out as arithmetic; driver + sweep in `experiments/28-dlt-analysis/`. Weights rule going forward: share ∝ 1/(p_i+l_i).
...
- `binary-frame-dist` → commit `aae49fb` — `DIST_*` static cyclic/block-cyclic frame distribution (PoC, experiments `25`); on `main` since the branch merge (`b342303`)
...
Twenty-six LaTeX reports exist (numbered 01–09, 11–18, 20–28; there are no reports 10/19; all on `main` since the `feat/frame-distribution` merge). Reports 20 and 22 measure the OpenCL iGPU backend whose *code* stays on the unmerged `feat/igpu-opencl` branch, but the reports and their `experiments/` data now live on `main`. Each report's title page pins the binary commit(s) that produced its measurements. Reports 01–13 are measurement reports; report 14 is a code/architecture guide and report 15 is an analysis/reconsideration (neither with `experiments/` data); reports 16–17 are kernel-level Nsight Compute measurements; report 18 is a split-rule decomposition characterization (16–18 each with `experiments/` data); report 21 is the periodicity-check measurement (experiment `21`); report 28 is a DLT modeling analysis (no new renderer measurements; experiment `28` holds the solver driver + outputs).
...
| `reports/25-frame-distribution.tex` | `feat/frame-distribution` `aae49fb`+`52c40c4` (tag `binary-frame-dist`), atop `main` `8acde54` (v7 lineage) | **Distributed frame rendering: mechanism, identity, heterogeneous 2-node benchmark.** The `DIST_*` static cyclic/block-cyclic frame-ownership predicate (frames across nodes, region queue intra-node) is **100/100 byte-identical in four executor-pinned checks**, including a real two-machine run (laptop + ivy: g++-15/Coffee Lake vs g++ 16/Haswell `GPU=0` build — plain `-O2`, no `-march`, no FMA contraction; adding `-march=native` would break it). Benchmark (3 reps, AC, same-day baseline 18.75±0.34 s): **every equal-share 2-node config loses to the laptop alone** — cyclic +150%, block-cyclic(8) +140%, block +247%, reversed block +51% best — because ivy is 4.56× slower per frame and equal shares strand the wall on it; block orientation is a built-in ~70/30 weighting (the zoom's cost trend, measured ≈70:30 on both machines). Analytic ceiling 1.22× (ideal weighted 15.4 s), half consumed by 3.4–4.2 s ship/collect overhead → a slow node buys **capacity, not latency**; dispenser necessary-but-barely-sufficient on this pair. Homogeneous block/cyclic/block-cyclic interleave study still open (heterogeneity buries the interleave effect). Baseline drift note: 18.75 s vs report 23's 23.72 s (environmental; same-day comparisons unaffected). |
| `reports/26-dynamic-balancing.tex` | `feat/frame-distribution` `ffe61aa`+`8c21bf4`, atop the report-25 lineage; data in `experiments/25-frame-distribution/results_dynamic{,_uncapped}.csv` | **Weighted shares + coordinator work stealing — the schedulers report 25's ceiling called for.** `DIST_WEIGHTS` seeded splitmix64 weighted-random static partition (communication-free, cross-machine identical) + `dist_dynamic.sh` stealing (guided chunks via `DIST_FRAMES`, empty node steals half the richest undispatched tail). Identity 100/100 at production scale incl. a **live corrective steal** (5:1 weights overload the CPU-pinned laptop — only 2.4× ivy without its GPU — and ivy steals 5 frames back: weights belong to the (machine, executor-mix) pair). Benchmark: calibrated 5:1 reaches **baseline parity** (weighted static 22.93±0.11, dynamic 22.01±0.56 vs 21.06±2.73 baseline; 2.1× over equal shares' 46.90); wrong 1:1 weights recover **72% of the miscalibration penalty autonomously** (46.90→28.18±0.37) — after the uncapped run exposed the unstealable in-flight chunk (25 frames = 27.4 s on ivy) and the `KCAP·w_r` cap fixed it (exposure = k/rate, rate ∝ weight → same wall-time budget per node, −9.9%, 2.7σ). With right weights stealing never fires (free insurance). Weighted/dynamic A/B on comparable nodes still open. |
| `reports/27-collection-batching.tex` | `feat/frame-distribution`, code state `c5bd3ed` (= report-26 code, docs-only atop it) on **both** nodes; Makefile autodetect `c8ac920` (build only), batching `1b5da5f` (scripts only), benchmarks/data `f310656`; data in `experiments/25-frame-distribution/results_bench_yeco.csv` + `results_dynamic_yeco{,_scp}.csv` | **Fast remote node + collection batching.** Reports 25/26 rerun with yeco (Ultra 7 265K 20T + RTX 4090, dichato SSH jump, ~25 ms RTT / ~37 MB/s) — **4.1× faster** than the laptop (solo 5.15±0.10 vs 21.02±1.94 s). Roles flip on cue: every equal-share config beats the laptop (best: cyclic 11.24±0.18) and loses to yeco solo; block orientation = the 70/30 weighting, now pointing at the fast node. **The surprise: weighted 1:4 computes at the 4.14 s harmonic ideal on both nodes yet e2e 13.11±1.81 — worse than equal shares** — balanced compute exposes per-file scp collection (~68 ms/file = 2-3 round trips; micro-A/B on the identical 79-file/36 MB share: scp 6.60 s vs one tar stream 0.96 s, byte-identical). Batching (`1b5da5f`) → weighted static **6.45±0.26 s** (−51%, σ ×7 tighter) = parity with yeco-solo+fetch: the capacity-not-latency ceiling realized a third time. Dynamic equalizes *lane time* (compute+ship): under scp the slow laptop steals 22 frames (lane-optimal 42/58); post-batch stealing = +21% chunking premium with right weights (CUDA context per chunk), and wrong 1:1 weights turn the `KCAP·w_r` cap against the fast node (9 dispatches / 63 frames) — mirror of report 26's in-flight defect; rate-adaptive caps identified as follow-up. |
| `reports/28-dlt-analysis.tex` | analysis only — inputs from report 27's data (`f310656`, code state `c5bd3ed`); solver = external `../DLTlib` (Barlas, GPL-3.0, §11.4 of the text); driver in `experiments/28-dlt-analysis/` | **DLT retrodiction of the yeco campaign.** Feeds the measured constants (p_laptop=0.2102, p_yeco=0.0515 s/frame; l_scp=83.5, l_tar=12.2 ms/frame; L=100) into DLTlib's collection-aware `SolveImage` (a=0: spec is free; c=1: PNGs proportional). Retrodicts everything: **l=0 reproduces our 1:4 weights** (80.3/19.7 — we ran DLT with the communication term deleted); scp optimum 39/61 predicts the collection wall (our 1:4 → 11.9 s model vs 13.11±1.81 measured); **the stealer's 42/58 scp equilibrium is 3 frames from the closed-form optimum** (lane-time self-scheduling = the model's finish-time equalization); post-batch 1:4 is 2% (sub-σ) off optimal — no re-weighting warranted; **transport beats scheduling in closed form** (optimally re-weighted scp 9.4 s still loses to naive-weights tar 6.2 s); 3-node outlook prices ivy at +4.8 frames / −0.24 s (sub-noise — capacity-not-latency as arithmetic). Validity: the load is linear only *on average* — the seeded random interleave is what satisfies DLT's uniform-cost premise (block assignment breaks it). Weights rule going forward: **share ∝ 1/(p_i+l_i)**, l recalibrated per transport. |
...
| `experiments/25-frame-distribution/` | `feat/frame-distribution` (`binary-frame-dist` + `52c40c4`) | `verify_dist.sh` (byte-identity: 3 simulated local nodes vs single process, executor-pinned CPU-only/GPU-only, cyclic + block-cyclic), `results_identity.csv`, `bench_2node.sh` (AC-gated laptop+ivy benchmark, 3 reps), `results_bench.csv`, `plot_bench25.py` (→ `reports/img/framedist_bench.png`, gitignored), `bench_dynamic.sh`/`results_dynamic.csv` + `results_dynamic_uncapped.csv` (weighted-static vs dynamic-stealing A/B, capped + pre-cap runs) and `plot_dynamic26.py` (→ `reports/img/framedist_dynamic.png`, gitignored), `bench_2node_yeco.sh`/`results_bench_yeco.csv` (laptop+yeco equal-share sweep), `bench_dynamic_yeco.sh`/`results_dynamic_yeco.csv` + `results_dynamic_yeco_scp.csv` (laptop+yeco weighted/dynamic A/B, post-/pre-batching) and `plot_yeco27.py` (→ `reports/img/framedist_yeco.png`, gitignored) — reports `25`+`26`+`27`; `collect/` + `verify_work/` + `bench/` + `bench_yeco/` scratch gitignored |
| `experiments/28-dlt-analysis/` | analysis (no renderer run); solver = external `../DLTlib` + GLPK | `mandel_dlt.cpp` (DLTdemo-style driver: 2-node `SolveImage` solves at l∈{0,tar,scp}, fixed-1:4 evaluation, steal endpoints, 3-node hand closed form), `run.sh` (copies DLTlib → `build/` gitignored, patches its `random.c` include, compiles with `-lglpk`, regenerates outputs), `results.txt` (solver table), `sweep.csv` (optimum share/wall vs l, 0–100 ms), `plot_dlt28.py` (→ `reports/img/dlt28_model.png`, gitignored), `README.md` — report `28` |
