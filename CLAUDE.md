...
Fifteen LaTeX reports document the work chronologically (numbered 01–09, 11–16; there is no report 10). Each report's title page pins the binary commit(s) that produced its measurements. Reports 01–13 are measurement reports; report 14 is a code/architecture guide and report 15 is an analysis/reconsideration (neither with associated `experiments/` data); report 16 is a kernel-level Nsight Compute measurement (with `experiments/16-ncu-divergence/`).
