...
| `experiments/25-frame-distribution/` | `feat/frame-distribution` (`binary-frame-dist` + `52c40c4`) | `verify_dist.sh` (byte-identity: 3 simulated local nodes vs single process, executor-pinned CPU-only/GPU-only, cyclic + block-cyclic), `results_identity.csv`, `bench_2node.sh` (AC-gated laptop+ivy benchmark, 3 reps), `results_bench.csv`, `plot_bench25.py` (→ `reports/img/framedist_bench.png`, gitignored), `README.md` — report `25`; `collect/` + `verify_work/` + `bench/` scratch gitignored |
