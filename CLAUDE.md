...
- `binary-v7-gpu-periodicity` → commit `2abc909` — exact Brent periodicity check in the GPU kernel `diverge()` (report `23`); on `feat/gpu-periodicity`, **not merged yet**. Best wall: hybrid 23.72 s (steady state), byte-identical output.
