...
Seventeen LaTeX reports document the work chronologically (numbered 01–09, 11–18; there is no report 10). Each report's title page pins the binary commit(s) that produced its measurements. Reports 01–13 are measurement reports; report 14 is a code/architecture guide and report 15 is an analysis/reconsideration (neither with `experiments/` data); reports 16–17 are kernel-level Nsight Compute measurements; report 18 is a split-rule decomposition characterization (16–18 each with `experiments/` data).
...
5. **Work-reduction — the remaining lever (scheduling is now exhausted).** With GPU affinity the GPU is ~91% utilized and the CPU pool is the wall; total compute (~595 s) is conserved and can only shrink by *skipping* iterations: (a) **periodicity checking** in `diverge()` — detect the cycle interior orbits settle into and bail early; exact, no image change, directly cuts the in-set pixel cost; (b) a **heuristic interior certificate** (constant-fill a region whose samples all reach `maxIter`) — faster but risks filling over thin filaments. The exact cardioid/bulb certificate doesn't apply (minibrot interiors). This is the recommended next branch (`feat/periodicity-check`). **Caveat from report `16` (ncu):** the GPU's coherent interior kernels are already **100% warp-efficient** and **FP64-bound at 85.6% of peak**, so periodicity checking would *introduce* warp divergence there (interior pixels bailing at different iterations) **and** add FP64 work to the bottleneck pipe — land it in `compute()`'s **CPU branch first**, then A/B the GPU branch separately. Warp divergence itself is a **non-lever** (measured 0% on the regions affinity actually routes to the GPU).
