...
4. ~~**Largest-first priority queue**~~ ❌ **Tried — wall-neutral, see `reports/11-priority-queue.tex` (branch `feat/priority-queue`, tag `binary-v4-pq`, NOT merged).** A shared largest-first `priority_queue` does not route big regions to the GPU: with 11 CPU threads vs 1 GPU thread, the next extractor is a CPU thread ~11:1, so the outlier stays on CPU (kernel max unchanged at ~139 ms). Wall −0.55% (noise). The real lever is **GPU affinity**: a double-ended (min-max) queue where the GPU thread pops the *largest* region and CPU threads pop the *smallest*, so the big coherent interior regions (≈30× faster on GPU, report 11 §3.4) run on the GPU. Pursued next on `feat/gpu-affinity` (report `12`).
